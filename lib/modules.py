...
class SpatialGate(nn.Module):
    def __init__(self):
        ...
        # 非对称直线卷积
        self.spatial_h = nn.Conv2d(2, 1, kernel_size=(1, 7), stride=1, padding=(0, 3))
        self.spatial_w = nn.Conv2d(2, 1, kernel_size=(7, 1), stride=1, padding=(3, 0))
    def forward(self, x):
        x_compress = torch.cat((torch.max(x,1)[0].unsqueeze(1), torch.mean(x,1).unsqueeze(1)), dim=1)
        x_out = self.spatial_h(x_compress) + self.spatial_w(x_compress)
...
class EGA(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(in_channels * 3, in_channels, kernel_size=3, stride=1, padding=2, dilation=2),
